...
! v4_14 - Dugas B.          - Read leap year control flag
...
!
      ...
      unf  = 0
      fn   = trim(Path_input_S)//'/MODEL_INPUT/Modntr_ipc.nml'
      ierr = wkoffit(fn)

      if (ierr.ge.-1) then
         if ( fnom (unf, fn, 'SEQ+OLD',ierr ) .eq. 0 ) then
            read (unf  ,nml=modipc_cfgs)
            Ana_horzint_L = horzint_L	
            call fclos (unf)	
            bin2com = 0	
         endif
      endif
      ...
      Tr3d_ntr_from_gemntr = Tr3d_ntr
      Tr3d_from_gemntr_S(1:Tr3d_ntr) = Tr3d_name_S(1:Tr3d_ntr)
