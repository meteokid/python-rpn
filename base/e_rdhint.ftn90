...
!
      integer function e_rdhint3 (   &
                  ...
                  typvar_S, anydate_L, fhint_L, interp_S, un1, stdo )
      implicit none
#include <arch_specific.hf>

      character* (*) nomvar_S,interp_S
      character* (*) etik_S,typvar_S ! NOT USED
      logical anydate_L,fhint_L
...
! v4_60 - Gravel S.         - replace anyip option by anydate_L
...
!                                 is required
      ...
      integer,external :: fstinf,fstprm,fstluk,ezqkdef,ezdefset,&
                          ezsetopt,ezsint,ezgetopt,ezsetival   ,&
                          ezgetival,ip1_all,samegrid_gid

      character*1  grd,typvar
      character*2  typ
      character*4  var
      character*12  lab,cdum,etiket
      character*32  interp2_S
      real    zp1
      logical tr_ip_L, must_interpo_L, same_grid_L
      integer dte, det, ipas, p1, p2, p3, g1, g2, g3, g4, bit, &
              dty, swa, lng, dlf, ubc, ex1, ex2, ex3
      integer i,key,nisrc,njsrc,nksrc,err,src_gid,iunit,kind,ni1, &
              nj1,subid,nicore,njcore
...
!
      ...
      if (ip1 .eq. -1 .or. ip1 .eq. 0 ) then
        key = fstinf (un1,nisrc,njsrc,nksrc,datev,etiket,&
                      ip1,ip2,ip3,typvar,nomvar_S)
      else
        call convip ( ip1, zp1, kind, -1, cdum, .false. )
        write(6,*)'e_rdhint3: ip1,zp1,kind= ',ip1,zp1,kind
        key = fstinf (un1,nisrc,njsrc,nksrc,datev,etiket,&
                ip1_all(zp1,kind),ip2,ip3,typvar,nomvar_S)
      endif

      tr_ip_L = key.ge.0

      if ( (.not.tr_ip_L) .and. (anydate_L) ) then
         call e_rdhint_out2(stdo,"Warning field not found: ",nomvar_S,ip1,ip2,ip3,' ')
         write(stdo,*) "Will try to locate it with with no specific datev"
         if (ip1 .eq. -1 .or. ip1 .eq. 0 ) then
           key = fstinf (un1,nisrc,njsrc,nksrc,-1,etiket,&
                                ip1,-1,-1,typvar,nomvar_S)
         else
           key = fstinf (un1,nisrc,njsrc,nksrc,-1,etiket,&
                ip1_all(zp1,kind),ip2,ip3,typvar,nomvar_S)
         endif
      ...
      if (grd.eq.'Z') then
         ...
         subid= samegrid_gid ( src_gid, Hgc_ig1ro,Hgc_ig2ro,&
                       Hgc_ig3ro,Hgc_ig4ro, xlon,ylat,ni,nj)
         if (subid >= 0) then
            interp2_S = 'NEAREST'
            same_grid_L = .true.
         endif
      ...
      if (grd.eq.'U') then
         ...
         !only one processor treating the model grid (yin and yang)

         nicore = G_ni-Glb_pil_w-Glb_pil_e ; njcore = G_nj-Glb_pil_s-Glb_pil_n
         
         if (nisrc >= nicore .and. njsrc/2 >= njcore) then
            subid= samegrid_gid ( src_gid, Hgc_ig1ro,Hgc_ig2ro,&
                        Hgc_ig3ro,Hgc_ig4ro, xlon(1+Glb_pil_w),&
                        ylat(1+Glb_pil_s),nicore,njcore )
         endif
      ...
      if ((interp2_S.eq.'NEAREST').and.(.not.same_grid_L)) then
         print*, 'CANNOT PROCEED'
      else
         call e_rdhint_out2 (stdo, "Horizontal interpolation: ", &
                             nomvar_S,ip1,p2,ip3,interp2_S)
         ...
         if (subid >= 0) then
            e_rdhint3 = 0 !Like no horizontal interpolation
            err = ezsetopt('USE_1SUBGRID','NO')
         endif
      endif

      if (interp2_S.ne.'NEAREST') then
         do i=1,ni*nj
