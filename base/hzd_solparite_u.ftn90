...
!        equation with mxma8 using reflexion symmetry of U-grid (global)

       subroutine hzd_solparite_U ( &
...
!
!$omp parallel private(kkii,pi0,pin,ki0,kin,gnii,nevi) shared (kilon)
!$omp do
      do k = 1, nkl
      do i = 1, Gni
      do j = 1, njl
...
!$omp do
      do i= 1,nev
         do k=1,Nkl
         do j=1,njl
            fdg12_8(j,k,i)=fdg1_8(j,k,i)+fdg1_8(j,k,Gni+1-i)
         enddo
         enddo
      enddo
!$omp enddo
...
!$omp do
      do i=1,Gni-nev
         do k=1,Nkl
         do j=1,njl
              fdg12_8(j,k,i+nev)=fdg1_8(j,k,i)-fdg1_8(j,k,Gni+1-i)
         enddo
         enddo
      enddo
!$omp enddo
...
!$omp do
      do k= 1,Nkl
        do j= 1,njl
           fdg1_8(j,k,nev)  = fdg12_8(j,k,nev)+fdg12_8(j,k,nevi)
           fdg1_8(j,k,gnii) = fdg12_8(j,k,nev)-fdg12_8(j,k,nevi)
        enddo
      enddo
!$omp end do
      endif
