...
fromversion=4.4.0-rc5
myversion=4.5.0-a1
doupcfg=1
fromPHYversion=5.4.1
toPHYversion=5.5.0
#TODO: extract blocstats (try to compare with previous version
...
myupcfg() {
	 ...
	 _here=$(pwd)
	 cd ${_dircfg}/cfg_0001
	 [[ x$doupcfg == x1 ]] && gem_upcfg --from ${fromversion} --to ${myversion}
    #TODO: integrate the phyversion update in gem_upcfg
    [[ x$fromPHYversion != x$toPHYversion ]] && mv gem_settings.nml gem_settings.nml-phy0 && cat gem_settings.nml-phy0 | sed "s/RPNPHY_${fromPHYversion}/RPNPHY_${toPHYversion}/" > gem_settings.nml
	 cd ${_here}
}
...
if [[ x$mycase == xall || x$mycase == xtheo || x$mycase == xtheo-mtn ]] ; then
