...
install_msgfile() {
	 ...
	 [[ x$_overwrite == x1 ]] && chmod 755 $bundledir/${bndl_dir0}/${ssm_dom_link_name}/$mymsgfile
	 cat $msgfile | \
...
for item in $ssm_compiler_list ; do
	 if [[ "x${item%:*}" == "xAIX" ]] ; then
		  mycomp="$(echo ${item#*:} | tr 'A-Z' 'a-z')"
		  export ssm_AIX_COMPILER="$mycomp"
	 ...
	 elif [[ "x${item%:*}" == "xAIX-powerpc7" ]]; then
		  mycomp="$(echo ${item#*:} | tr 'A-Z' 'a-z')"
		  export ssm_AIX_powerpc7_COMPILER="$mycomp"
	 fi
done
...
[[ x$bndl_dir1 == x$bndl_name ]] && bndl_dir1='./'
...
for item in $ssm_bndl_elements ; do
	 ...
	 if [[ x$item2 != x ]] ; then
		  ...
		  if [[ "x$myarch" == "xmsg:" ]] ; then
				...
				if [[ "x$myarch" == "xall:" || "x$allonly" != "x1" ]] ; then
