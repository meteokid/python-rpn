subroutine flipit()
   ...
   character*1024 LISTEc(2), DEF(2), VAL(2)
   data LISTEc /'i.'  , 'o.'/
   data VAL    /'null'  , 'null'/
   data DEF    /'null'  , 'null'/

   character*1024 out_file,in_file
   character*12   etiket
   character*4    nomvar,liste_var(50000)
   character*2    typvar
   character*1    grtyp,blk_S

   integer, parameter ::nlis = 10024
   integer liste(nlis), liste2(nlis), liste3(nlis)

   integer dateo,  deet, npas, nbits, NPOS
   ...
   if (fnom(iun1,in_file,'RND+OLD',0).ge.0) then
   ...
   if (fnom(iun2,out_file,'rnd',0).ge.0) then
