...
#include <rmnlib_basics.hf>
      ...
      integer ut1_is_urt1,err_fstlir
...
!
      ...
      if (Inp_iome .ge. 0) then
         ...
         if ((trim(nomvar) == 'URT1').or.(trim(nomvar) == 'VRT1').or.&
             ...
             err= samegrid_gid ( src_gid, Hgc_ig1ro,Hgc_ig2ro,Hgc_ig3ro,Hgc_ig4ro,&
                                 posx(1+Grd_extension), posy(1+Grd_extension)    ,&
                                 G_ni-2*Grd_extension, G_nj-2*Grd_extension )
             else
 ...
 999  call rpn_comm_bcast ( lislon, 2, "MPI_INTEGER", Inp_iobcast, &
                            "grid", err )
      call gem_error (err_fstlir,'ERROR with FSTLIR','inp_read')
