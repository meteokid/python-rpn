...
!**s/r int_cubv_lag - YY cubic interpolation for vector fields (local PE)

       subroutine int_cubv_lag ( FF, Fu_8, Fv_8,Imx1,Imy1, Imx2, Imy2, &
...
!For V
       ...
       FF((i-1)*Nk+k)=real(s2_8(i)*FFv_8 + s1_8(i)*FFu_8)
  300  continue
  200  continue
!$omp enddo
!$omp end parallel
