...
#include "phymkptr.hf"
      ...
      real, dimension(ni,nkm1) :: lwpinmp, cldfmp, lwcinmp, iwpinmps, iwcinmps
      ...
      call init2nan(lwpinmp, cldfmp, lwcinmp, iwpinmps, iwcinmps)
      ...
      DO_K: do k=1,nkm1
         do i=1,ni
            ...
            if (cldfrac(i,k) < 0.01) then
               liqwcin(i,k) = 0.
               icewcin(i,k) = 0.
               cldfrac(i,k) = 0.
            else
               liqwcin(i,k) = liqwcin(i,k)/cldfrac(i,k)
               icewcin(i,k) = icewcin(i,k)/cldfrac(i,k)
            endif
            ...
            if (cldfmp(i,k) < 0.01) then
               ...
               lwcinmp(i,k) = lwcinmp(i,k)/cldfmp(i,k)
               do l=1,mpcat
                  iwcinmp(i,k,l) = iwcinmp(i,k,l)/cldfmp(i,k)
               enddo
            endif
               ...
               IF_NOCLOUD2: if (nocloud(i,k)) then
                  ...
                  if (lwpinmp(i,k) > 0.001) then
                     ...
                     if (taulwmp<0.) then
                        ...
                        call physeterror('cldoppro_MP', 'Found negative values.')
                        return
                     endif
