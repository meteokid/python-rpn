...
!
      subroutine out_dyn_3df_2 ( datev )
      implicit none
...
#include "ptopo.cdk"
      ...
      integer i,j,k,nis,njs,cnt,unf,nvar,err,ofi,ofj,istat,ntra
      integer outindx(4),outindx_g(4,Ptopo_numproc),gid,gif,gjd,gjf
...
!
      ...
      gid= Grdc_gid ; gif= Grdc_gif
      gjd= Grdc_gjd ; gjf= Grdc_gjf
      ...
      if (Out_blocme.eq.0) call out_sfile_3df3 ( datev, unf )
      ...
      do k=1,Grdc_ntr
         ...
         call write_3df3 ( tr2,l_minx,l_maxx,l_miny,l_maxy,nis,njs,G_nk,&
                           Grdc_trnm_S(k), gid, gif, gjd, gjf, unf, done )
      end do

      if ( out3_sfcdiag_L ) then
         ntra= 3+Grdc_ntr
         allocate(tr1(l_minx:l_maxx,l_miny:l_maxy,ntra))
         tr1=0.
         ...
         do k=1,Grdc_ntr
            nullify (tr2)
            call itf_phy_sfcdiag(tr1(l_minx,l_miny,k+3),&
                            l_minx,l_maxx,l_miny,l_maxy,&
                            'TR/'//trim(Grdc_trnm_S(k))//':P',istat,.true.)
         end do

         call write_3df3 (tr1,l_minx,l_maxx,l_miny,l_maxy,nis,njs,ntra,&
