...
!
      ...
      if (VAR_TT.eq.'TT1') c1 = 0.
      vhh= VAR_HU//'  '
      if (VAR_TT.eq.'TT  ') then
          vtt='TT  '
      else 
          vtt='VT  '
      endif
...
!
      if (.not.Anal_pres_L) then
         ...
         code_h= e_rdhint3 (p0,dstf_gid,nis,njs,'ST1 ',-1, &
              -1,-1,' ',' ',.false.,.false.,'CUBIC',e_fu_anal,6)
...
! Temperature and humidity
         ...
         do k=1,lv_t
            kk=k
            if (Pil_bmf_L) kk=1
            code_h= e_rdhint3 (hun(1,kk),dstf_gid,nis,njs,'HU  ',na_t(k),-1, &
                -1,' ',' ',.false.,.false.,'CUBIC',e_fu_anal,6)
            if (code_h.lt.0) goto 999
            horinterp_L= horinterp_L .or. (code_h.gt.0)

            code_h= e_rdhint3(ttn(1,kk),dstf_gid,nis,njs,VAR_TT,na_t(k), -1, &
                -1,' ',' ',.false.,.false.,'CUBIC',e_fu_anal,6)
            ...
            if (Pil_bmf_L) then
               ...
               call e_bmfsplitxy2 (ttn,nis,njs,vtt,k,lv_t,pni,0,0,0)
            endif
         end do         
         ...
         if (VAR_ZD /= '') then
            do k=1,lv_t-1
               kk=k
               if (Pil_bmf_L) kk=1
               code_h= e_rdhint3 (zdn(1,kk),dstf_gid,nis,njs,VAR_ZD,na_t(k),-1    , &
                    -1,' ',' ',.false.,.false.,'CUBIC',e_fu_anal,6)
               if (code_h.lt.0) goto 999
               horinterp_L= horinterp_L .or. (code_h.gt.0)
               if (Pil_bmf_L) then
         ...
         if (VAR_W /= '') then
            do k=1,lv_t-1
               kk=k
               if (Pil_bmf_L) kk=1 
               code_h= e_rdhint3 (wn(1,kk),dstf_gid,nis,njs,VAR_W,na_t(k),-1, &
                    -1,' ',' ',.false.,.false.,'CUBIC',e_fu_anal,6)
               if (code_h.lt.0) goto 999
               horinterp_L= horinterp_L .or. (code_h.gt.0)
               if (Pil_bmf_L) then
         ...
         ip1_qt(1:lv-1)=na(2:lv)
         call convip(ip1_qt(lv-1),1.,5,+2,'',.true.)

         if (VAR_Q /= '') then
            do k=1,lv-1
               kk=k
               if (Pil_bmf_L) kk=1 
               code_h= e_rdhint3 (qn(1,kk),dstf_gid,nis,njs,'QT1 ',ip1_qt(k),-1,&
                                  -1,' ',' ',.false.,.false.,'CUBIC',e_fu_anal,6)
               if (code_h.lt.0) goto 999
               horinterp_L= horinterp_L .or. (code_h.gt.0)
               if (Pil_bmf_L) then
                  call e_bmfsplitxy2 (qn,nis,njs,'Q   ',k,lv-1,pni,0,0,0)                
               endif
            enddo
         endif
         ...
         if (Pil_bmf_L) then
            ...
            call e_write_3df ( ttn,nis,njs, lv_t  , vtt   , unf_casc )
         ...
         do k=1,lv_t
            ...
            code_h= e_rdhint3 (gzn(1,kk),dstf_gid,nis,njs,'GZ  ',na_t(k),-1, &
                -1,' ',' ',.false.,.false.,'CUBIC',e_fu_anal,6)
            if (code_h.lt.0) goto 999
            horinterp_L= horinterp_L .or. (code_h.gt.0)         
            ...
            code_h= e_rdhint3 (wk2,dstf_gid,nis,njs,vhh,na_t(k),-1, &
                -1,' ',' ',.false.,.false.,'CUBIC',e_fu_anal,6)
            if (code_h.lt.0) goto 999
            horinterp_L= horinterp_L .or. (code_h.gt.0)         
            ...
            ! WARNING: These next conversions are probably incorrect if (VAR_TT.eq.'VT')
            if (VAR_HU.eq.'HR') then
               call mhrahu3( hun(1,kk), wk2, ttn(1,kk), rna_tlev, &
                             Anal_cond_L, ng, 1, ng )
            elseif (VAR_HU.eq.'ES') then
               call mesahu3( hun(1,kk), wk2, ttn(1,kk), rna_tlev, &
                             Anal_cond_L, ng, 1, ng )
            elseif (VAR_HU.eq.'TD') then
            ...
            if (VAR_TT.eq.'TT') call mfotvt (ttn(1,kk),ttn(1,kk),hun(1,kk),ng,1,ng)
         ...
         if (.not.Pil_bmf_L) then
            call e_write_3df ( ttn,nis,njs,lv_t  ,'VT  ',unf_casc)
...
!     They can be assigned a constant value.
      ...
      do i=1, Tr3d_ntr
         do k=1,lv_t
            ...
            if (Pil_bmf_L) then
               call e_bmfsplitxy2 (hun,nis,njs,Tr3d_name_S(i),k,lv_t,pni,0,0,0)
            endif
         enddo
         if (.not.Pil_bmf_L) then
            call e_write_3df ( hun,nis,njs,lv_t,Tr3d_name_S(i),unf_casc )
         endif 
      end do

      keyt= fstinf (e_fu_anal,nic,njc,nkc,datev,' ',na_t(2), -1,-1,' ','PWTT')
      if (keyt.ge.0) then
         err= 0
         do k=2,lv_t-1
            kk=k
            if (Pil_bmf_L) kk=1
            keyt = e_rdhint3 (qn(1,kk),dstf_gid,nis,njs,'PWTT',na_t(k),-1, &
                           -1,' ',' ',.false.,.false.,'NEAREST',e_fu_anal,6)
            if (keyt.ne.0) goto 999
            if (Pil_bmf_L) then
               call e_bmfsplitxy2 (qn,nis,njs,'PWTT',k-1,lv_t-1,pni,0,0,0)
            endif
         end do
         if (.not.Pil_bmf_L) then
            call e_write_3df ( qn,nis,njs,lv_t-2,'PWTT',unf_casc)
         endif         
      endif
