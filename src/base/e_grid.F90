...
#include "lun.cdk"
      ...
      real*8, parameter :: ONE_8  = 1.0  , CLXXX_8 = 180.0
!
!----------------------------------------------------------------------
!
      if (Lun_out.gt.0) write(6,1001)

      deg2rad_8 = acos( -ONE_8 )/CLXXX_8

      global_extend = (abs(1.0d0-abs(Grd_xl_8-Grd_x0_8)/360.d0).lt.epsilon) &
                 .or. (abs(1.0d0-abs(Grd_yl_8-Grd_y0_8)/180.d0).lt.epsilon)

      call handle_error_l(.not.(lam .and. global_extend),'e_grid','LAM grid extends to global coverage on at least one axis')
      ...
      G_lam = lam
      G_periodx = (.not.G_lam) ; G_periody = .false.
      G_ni = nifi ; G_nj = njfi ; G_nk = 1 ; G_niu = niu ; G_njv = njv
      G_lnimax = G_ni ; G_lnjmax  = G_nj ; G_halox = 0 ; G_haloy = 0
      G_xg_8 => xdx ; G_xg_8 => xdy
      l_north = .true. ; l_south = .true. ; l_east = .true. ; l_west = .true.
      l_ni = nifi ; l_nj = njfi ; l_nk = 1 ; l_niu = niu ; l_njv = niv
      l_i0 = 1 ; l_j0 = 1 
      l_minx = 1 ; l_maxx = l_ni ; l_miny = 1 ; l_maxy = l_nj
      pil_w = 0 ; pil_n = 0 ; pil_e = 0 ; pil_s = 0
      north = 1 ; south = 1 ; east = 1 ; west = 1
      Glb_pil_w = Glb_pil_n ; Glb_pil_e = Glb_pil_n
      Glb_pil_w = 0 ; Glb_pil_n = 0 ; Glb_pil_e = 0 ; Glb_pil_s = 0
      Lam_pil_w = 0 ; Lam_pil_n = 0 ; Lam_pil_e = 0 ; Lam_pil_s = 0
      if (G_lam) then
         Glb_pil_n = Grd_extension ; Glb_pil_s = Glb_pil_n
         Glb_pil_w = Glb_pil_n ; Glb_pil_e = Glb_pil_n
         Lam_pil_w = Glb_pil_w ; Lam_pil_n = Glb_pil_n
         Lam_pil_e = Glb_pil_e ; Lam_pil_s = Glb_pil_s
