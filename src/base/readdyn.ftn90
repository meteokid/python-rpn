...
!
      subroutine readdyn4 ( F_u, F_v, F_w, F_t, F_zd, F_s, F_q, F_topo, &
...
#include "wil_williamson.cdk"
      ...
      logical initial_data,flag_sfcpres
      ...
      real, allocatable, dimension(:,:  ) :: psu_temp,psv_temp,apsu_temp,apsv_temp
      real, allocatable, dimension(:,:,:) :: u_temp,v_temp, hu_temp, tt_temp, &
                                               gz_temp,zd_temp,w_temp,q_temp, &
                                               u_temp2,v_temp2,work, vt_temp,pres
...
!--------------------------------------------------------------------
      ...
      if (Acql_prsanl) then   ! ANALYSIS IS ON PRESSURE COORDINATES
         ...
         allocate ( pres(l_ni,l_nj,nka_t), vt_temp(l_ni,l_nj,nkmax) )
         err= get_px (pres,ps,nbpts,zat_8,zbt_8,nka_t,Acqi_datasp(2),.false.)
         call handle_error(err,'readdyn','readdyn:get_px')

         if (Acqi_datasp(2).eq.6) then
            flag_sfcpres= .false. ! do it only if necessary
            do j=1,l_nj
            do i=1,l_ni
               if (abs(pres(i,j,nka_t)-Cstv_pref_8*exp(ps(i,j))) / &
                       pres(i,j,nka_t) .gt. 1.e-6) flag_sfcpres= .true.
            enddo
            end do
            if (flag_sfcpres) pres(:,:,nka_t)= Cstv_pref_8*exp(ps(:,:))
         endif
...
!---------------------------------------------------------------------
      ...
      if (Acql_vertint) then
         ...
         err = get_px (srclev_t,ps ,l_ni*l_nj,zat_8,zbt_8,nka_t,Acqi_datasp(2),.true.)
         call handle_error(err,'readdyn','readdyn:get_px')
         err = get_px (dstlev_t,pps,l_ni*l_nj,Ver_a_8%t,Ver_b_8%t,G_nk, &
                       Ver_code,.true.)
         call handle_error(err,'readdyn','readdyn:get_px')

         allocate ( srclev_m(l_ni,l_nj,nka_m), dstlev_m(l_ni,l_nj,G_nk+1) )
         err = get_px (srclev_M,ps ,l_ni*l_nj,za_8,zb_8,nka_m,Acqi_datasp(2),.true.)
         call handle_error(err,'readdyn','readdyn:get_px')
...
!---------------------------------------------------------------------
      ...
      do k=1,Tr3d_ntr
         ...
         if ( err.eq.0 ) then
            if (Acql_vertint) then
               call vertint ( work,dstlev_t,G_nk, hu_temp,srclev_t,nka_t, &
                              1,l_ni,1,l_nj, 1,l_ni,1,l_nj,'cubic',.false.)
               if (Schm_bitpattern_L) then
                  tr (1:l_ni,1:l_nj,1:G_nk) = work   (1:l_ni,1:l_nj,1:G_nk)
               else
                  tr (1:l_ni,1:l_nj,1:G_nk) = max(work   (1:l_ni,1:l_nj,1:G_nk), 0.)
               endif
            else
               if (Schm_bitpattern_L) then
                  tr (1:l_ni,1:l_nj,1:G_nk) = hu_temp(1:l_ni,1:l_nj,1:G_nk)
               else
                  tr (1:l_ni,1:l_nj,1:G_nk) = max(hu_temp(1:l_ni,1:l_nj,1:G_nk), 0.)
               endif
            endif
         else
            tr (1:l_ni,1:l_nj,1:G_nk) = 0.
         endif
      end do

!     TEMPERATURE TT1
      if (Acql_vertint) then
         call vertint ( work,dstlev_t,G_nk, tt_temp,srclev_t,nka_t,&
                        1,l_ni,1,l_nj, 1,l_ni,1,l_nj,'cubic',.true.)
      else
         work(:,:,:)=tt_temp(:,:,:)
      endif
      if (ana_vt_l) then
         ...
         nullify(hu)
         tr_name = trim(pref_tr)//'HU'//trim(suff_tr)
         istat = gmm_get(tr_name,hu,mymeta)
         call sumhydro(qh,l_minx,l_maxx,l_miny,l_maxy,G_nk,'P')
         if (pref_tr(1:5) == 'NEST/') qh= 0. ! Must fix sumhydro or tt2virt
      ...
      if (ana_zd_L) then
         if (Acql_vertint) then
            call vertint ( work,dstlev_t,G_nk, zd_temp,srclev_t,nka_t-1,&
                           1,l_ni,1,l_nj, 1,l_ni,1,l_nj,'cubic',.false.)
            F_zd(1:l_ni,1:l_nj,1:G_nk) = work(1:l_ni,1:l_nj,1:G_nk)
         else
      ...
      if (ana_w_L) then
         if (Acql_vertint) then
            call vertint ( work,dstlev_t,G_nk, w_temp,srclev_t,nka_t-1,&
                           1,l_ni,1,l_nj, 1,l_ni,1,l_nj,'cubic',.false.)
      ...
      if (ana_q_L) then
         if (Acql_vertint) then
            call vertint ( work,dstlev_m(1,1,2),G_nk, q_temp,srclev_m(1,1,2),nka_m-1,&
                           1,l_ni,1,l_nj, 1,l_ni,1,l_nj,'cubic',.false.)
            F_q(1:l_ni,1:l_nj,2:G_nk+1) = work  (1:l_ni,1:l_nj,1:G_nk)
         else
            F_q(1:l_ni,1:l_nj,2:G_nk+1) = q_temp(1:l_ni,1:l_nj,1:G_nk)
         endif
      endif
...
!       Get pressure on the U,V destination model grid
        ...
        allocate ( srclev_m (Acqi_niu,Acqi_nju,nka_m), &
                   dstlev_m (Acqi_niu,Acqi_nju,G_nk) )

        err = get_px (srclev_m,apsu_temp,Acqi_niu*Acqi_nju,za_8,zb_8,&
                      nka_m,Acqi_datasp(2),.true.)
        call handle_error(err,'readdyn','readdyn:get_px')
        err = get_px (dstlev_m,psu_temp ,Acqi_niu*Acqi_nju,Ver_a_8%m,&
                      Ver_b_8%m,G_nk,Ver_code,.true.)
        call handle_error(err,'readdyn','readdyn:get_px')
        call vertint ( u_temp2,dstlev_m,G_nk, u_temp,srclev_m,nka_m,&
                       1,Acqi_niu,1,Acqi_nju, 1,Acqi_niu,1,Acqi_nju,&
                       'cubic',.false.)
        deallocate (srclev_m,dstlev_m)

        allocate ( srclev_m (Acqi_niv,Acqi_njv,nka_m), &
                   dstlev_m (Acqi_niv,Acqi_njv,G_nk) )

        err = get_px (srclev_m,apsv_temp,Acqi_niv*Acqi_njv,za_8,zb_8,&
                      nka_m,Acqi_datasp(2),.true.)
        call handle_error(err,'readdyn','readdyn:get_px')
        err = get_px (dstlev_m,psv_temp ,Acqi_niv*Acqi_njv,Ver_a_8%m,&
                      Ver_b_8%m,G_nk,Ver_code,.true.)
        call handle_error(err,'readdyn','readdyn:get_px')
        call vertint ( v_temp2,dstlev_m,G_nk, v_temp,srclev_m,nka_m,&
                       1,Acqi_niv,1,Acqi_njv, 1,Acqi_niv,1,Acqi_njv,&
                       'cubic',.false.)
        do k=1,G_nk
      ...
      do k=1,G_nk
         ...
         F_v(1:Acqi_niv,1:Acqi_njv,k) = v_temp(1:Acqi_niv,1:Acqi_njv,k)
      end do
      ...
      if (Schm_phyms_L) then
         ...
         if ((err_pwuu.eq.0).and.(err_pwvv.eq.0)) then
            ...
            do k= 1, G_nk
               pw_uu_plus(1:l_ni,1:l_nj,k) = hu_temp(1:l_ni,1:l_nj,k)
               pw_vv_plus(1:l_ni,1:l_nj,k) = tt_temp(1:l_ni,1:l_nj,k)
            end do
            Acql_pwuv= .true.
         endif
         ...
         if (err_pwtt.eq.0) then
            ...
            do k= 1, G_nk
               pw_tt_plus(1:l_ni,1:l_nj,k) = hu_temp(1:l_ni,1:l_nj,k)
            end do
            Acql_pwtt= .true.
         endif
