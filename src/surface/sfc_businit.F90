...
#include "phymkptr.hf"
   ...
   PHYVAR3D1(sfcwgt,       'VN=sfcwgt       ;ON=WT  ;VD=wgt of sfc type over a tile                    ;VS=A*'//nagg//'    ;VB=v0        ;MIN=0')
   ...
   PHYVAR3D1(yradsun,      "VN=yradsun      ;ON=RTSU;VD=MRT in the exposed sunny street (K)                       ; VS=A*"//nagg//" ; VB=p0")
   PHYVAR3D1(yradshade,    "VN=yradshade    ;ON=RTHD;VD=MRT in the shaded street (K)                              ; VS=A*"//nagg//" ; VB=p0")
   PHYVAR3D1(yutcisun,     "VN=yutcisun     ;ON=DXSU;VD=UTCI in the exposed sunny street (C)                      ; VS=A*"//nagg//" ; VB=p0")
   PHYVAR3D1(yutcishade,   "VN=yutcishade   ;ON=DXHD;VD= UTCI in the shaded street (C)                            ; VS=A*"//nagg//" ; VB=p0")
   PHYVAR3D1(ywbgtsun,     "VN=ywbgtsun     ;ON=GXSU;VD= WBGT in the exposed sunny street (C)                     ; VS=A*"//nagg//" ; VB=p0")
   PHYVAR3D1(ywbgtshade,   "VN=ywbgtshade   ;ON=GXHD;VD= WBGT in the shaded street (C)                            ; VS=A*"//nagg//" ; VB=p0")
   PHYVAR3D1(ytglbsun,     "VN=ytglbsun     ;ON=GTSU;VD=TGlobe in the exposed sunny street (K)                    ; VS=A*"//nagg//" ; VB=p0")
   PHYVAR3D1(ytglbshade,   "VN=ytglbshade   ;ON=GTHD;VD=TGlobe in the shaded street (K)                           ; VS=A*"//nagg//" ; VB=p0")
   PHYVAR3D1(ytwetb,       "VN=ytwetb       ;ON=WBT ;VD=Wet-Bulb Temp at zt above the ground (K)                  ; VS=A*"//nagg//" ; VB=p0")
   PHYVAR3D1(yQ1,          "VN=yQ1          ;ON=QSSU;VD= Contribution of direct solar rad (W/m2)                  ; VS=A*"//nagg//" ; VB=p0")
   PHYVAR3D1(yQ2,          "VN=yQ2          ;ON=QSSK;VD= Contribution of sky SW rad (W/m2)                        ; VS=A*"//nagg//" ; VB=p0")
   PHYVAR3D1(yQ3,          "VN=yQ3          ;ON=QLSK;VD= Contribution of sky LW rad (W/m2)                        ; VS=A*"//nagg//" ; VB=p0")
   PHYVAR3D1(yQ4,          "VN=yQ4          ;ON=QSRD;VD= Contribution of ground SW rad (W/m2)                     ; VS=A*"//nagg//" ; VB=p0")
   PHYVAR3D1(yQ5,          "VN=yQ5          ;ON=QLRD;VD= Contribution of ground LW rad (W/m2)                     ; VS=A*"//nagg//" ; VB=p0")
   PHYVAR3D1(yQ6,          "VN=yQ6          ;ON=QSWL;VD= Contribution of facet SW rad (W/m2)                      ; VS=A*"//nagg//" ; VB=p0")
   PHYVAR3D1(yQ7,          "VN=yQ7          ;ON=QLWL;VD= Contribution of facet LW rad (W/m2)                      ; VS=A*"//nagg//" ; VB=p0")
...
! check/add min values !!!
   ...
   PHYVAR2D1(yradin,       'VN=yradin       ;ON=RTIN;VD=MRT inside building  (K)                                  ;VB=p0')
   PHYVAR2D1(yradrfsun,    'VN=yradrfsun    ;ON=RTFS;VD=MRT on the exposed sunny roof (K)                         ;VB=p0')
   PHYVAR2D1(yradrfshade,  'VN=yradrfshade  ;ON=RTFD;VD=MRT on the shaded roof (K)                                ;VB=p0')
   PHYVAR2D1(yutciin,      'VN=yutciin      ;ON=DXIN;VD=UTCI inside building  (C)                                 ;VB=p0')
   PHYVAR2D1(yutcirfsun,   'VN=yutcirfsun   ;ON=DXFS;VD=UTCI on the exposed sunny roof (C)                        ;VB=p0')
   PHYVAR2D1(yutcirfshade, 'VN=yutcirfshade ;ON=DXFD;VD=UTCI on the shaded roof (C)                               ;VB=p0')
   ...
   PHYVAR2D1(ywbgtrfsun,   'VN=ywbgtrfsun   ;ON=GXFS;VD=WBGT on the exposed sunny roof (C)                        ;VB=p0')
   PHYVAR2D1(ywbgtrfshade, 'VN=ywbgtrfshade ;ON=GXFD;VD=WBGT on the shaded roof (C)                               ;VB=p0')
   PHYVAR2D1(yutcicin,     'VN=yutcicin     ;ON=DCIN;VD=cumulative UTCI inside building  (C)                      ;VB=p0')
   PHYVAR2D1(yutcicsun,    'VN=yutcicsun    ;ON=DCSU;VD=cumulative UTCI in the exposed sunny street               ;VB=p0')
   PHYVAR2D1(yutcicshade,  'VN=yutcicshade  ;ON=DCHD;VD=cumulative UTCI in the shaded street (C)                  ;VB=p0')
   PHYVAR2D1(yutcicrfsun,  'VN=yutcicrfsun  ;ON=DCFS;VD=cumulative UTCI on the exposed sunny roof (C)             ;VB=p0')
   PHYVAR2D1(yutcicrfshade,'VN=yutcicrfshade;ON=DCFD;VD=cumulative UTCI on the shaded roof (C)                    ;VB=p0')
   PHYVAR2D1(ytglbrfsun,   'VN=ytglbrfsun   ;ON=GTFS;VD=TGlobe on the exposed sunny roof (K)                      ;VB=p0')
   PHYVAR2D1(ytglbrfshade, 'VN=ytglbrfshade ;ON=GTFD;VD=TGlobe on the shaded roof (K)                             ;VB=p0')
   PHYVAR2D1(ytwetbrf,     'VN=ytwetbrf     ;ON=WBRF;VD=Wet-Bulb Temperature at zt above the roof (K)             ;VB=p0')
   PHYVAR2D1(yQ8,          'VN=yQ8          ;ON=QSRF;VD=Contribution of roof SW rad (W/m2)                        ;VB=p0')
   PHYVAR2D1(yQ9,          'VN=yQ9          ;ON=QLRF;VD=Contribution of roof LW rad (W/m2)                        ;VB=p0')
   PHYVAR2D1(yQ10,         'VN=yQ10         ;ON=QSFK;VD=Contribution of sky on the roof SW rad (W/m2)             ;VB=p0')
   PHYVAR2D1(yQ11,         'VN=yQ11         ;ON=QLFK;VD=Contribution of sky on the roof LW rad (W/m2)             ;VB=p0')
   PHYVAR2D1(yQ12,         'VN=yQ12         ;ON=QSFW;VD=Contribution of wall on the roof SW rad (W/m2)            ;VB=p0')
   PHYVAR2D1(yQ13,         'VN=yQ13         ;ON=QLFW;VD=Contribution of wall on the roof LW rad (W/m2)            ;VB=p0')
