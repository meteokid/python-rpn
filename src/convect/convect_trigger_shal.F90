...
REAL_B, DIMENSION(KLON) :: ZCAP     ! pseudo for CAPE
...
!INTEGER_M, DIMENSION(KLON) :: ITOP  ! work array to store highest test layer
...
DO JK = IKB + 1, IKE - 2
 ...
 ! ENDDO
   IF ( PZ(1,JK) - PZ(1,IKB) < 5.E3_JPRB ) JT = JK
ENDDO
...
!                  &       * SQRT( PDXDY(JI) / XA25 )
!                  &      - 0.02_JPRB  * ZZLCL(JI) / XZLCL ! avoid spurious convection
...
!    END DO
!    WHERE( GWORK1(:) )
!      ZWLCL(:)  = _ONE_ + .5_JPRB * ZWORK2(:) * SQRT( ABS( ZWORK3(:) ) )
     ...
     DO JL = JKM, JT
        ...
        DO JI = 1, IIE
         IF ( GWORK1(JI) ) THEN
           ...
           ZCAPE(JI)  = ZCAPE(JI) + RG * MAX( _ZERO_, ZWORK1(JI) )
           ZWORK2(JI) = XNHGAM * RG * ZCAP(JI) + 1.05_JPRB * ZWLCL(JI) * ZWLCL(JI)
