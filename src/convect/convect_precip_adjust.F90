...
REAL_B, DIMENSION(KLON),      INTENT(IN) :: PUTPR ! updraft  total precipit. (kg/s
REAL_B, DIMENSION(KLON),      INTENT(IN) :: PPREF ! precipitation efficiency
REAL_B, DIMENSION(KLON),      INTENT(IN) :: PMIXF ! critical mixed fraction at LCL
...
INTEGER_M, DIMENSION(KLON),   INTENT(IN) :: KETL  ! contains vert. index of equilibrium
                                                  ! (zero buoyancy) level
INTEGER_M, DIMENSION(KLON),  INTENT(INOUT) :: KLFS ! contains vert. index of LFS
INTEGER_M, DIMENSION(KLON),  INTENT(INOUT) :: KDBL ! contains vert. index of DBL

REAL_B, DIMENSION(KLON),      INTENT(INOUT) :: PDTEVR ! total downdraft evaporation
                                                      ! rate at LFS
...
REAL_B, DIMENSION(KLON,KLEV), INTENT(INOUT) :: PUPR   ! updraft  precipit. (kg/s)
...
REAL_B, DIMENSION(KLON,KLEV), INTENT(INOUT) :: PDTHL  ! downdraft enthalpy (J/kg)
REAL_B, DIMENSION(KLON,KLEV), INTENT(INOUT) :: PDRW   ! downdraft total water (kg/kg)

REAL_B, DIMENSION(KLON),     INTENT(OUT)   :: PTPR    ! total precipitation (kg/s)
                                                      ! = downdraft precipitation

!*       0.2   Declarations of local variables :

INTEGER_M :: IIE, IKB, IKE        ! horizontal + vertical loop bounds
INTEGER_M :: JK, JKT1, JKT2, JKT3 ! vertical loop index
INTEGER_M :: JI                   ! horizontal loop index

INTEGER_M, DIMENSION(KLON) :: IPRL
REAL_B, DIMENSION(KLON)    :: ZWORK1, ZWORK2, ZWORK3, &
...
DO JI = 1, IIE
     JK = KLFS(JI)
     ZWORK1(JI) = PDTEVR(JI) / MIN( -1.E-1_JPRB, PDMF(JI,JK) )
...
DO JI = 1, IIE
     ...
     ZWORK2(JI) = ( _ONE_ - PPREF(JI) ) * ZWORK5(JI) *                     &
                & ( _ONE_ - PMIXF(JI) ) / MAX( 1.E-1_JPRB, PUMF(JI,JK) )
ENDDO
...
ZWORK1(:) = ZWORK4(:) / ( ZWORK1(:) + ZWORK2(:) + 1.E-8_JPRB )
ZWORK2(:) = ZWORK1(:) / MIN( -1.E-1_JPRB, ZWORK6(:) ) ! ratio of budget consistent to actual DMF
