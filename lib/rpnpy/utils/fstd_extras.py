#!/usr/bin/env python
# -*- coding: utf-8 -*-
# Author: Mike Neish <michael.neish@canada.ca>
# Copyright: LGPL 2.1

"""
Utility functions to speed up decoding parameters from RPN standard files.
"""
...
def stamp2datetime (date):
  """
  Converts an RPN date stamp to datetime object.
  Returns None for invalid stamps.
  """
  import numpy as np
  return np.asarray(_stamp2datetime(date), dtype='datetime64[ns]')
...
def decode_ip1 (ip1):
  """
  Decodes ip1 information
  """
  import numpy as np
  result = _decode_ip1(ip1)
  if not hasattr(ip1,'__len__'):
    return result[0]
  result = np.concatenate(result)
  return result


#NOTE: The following relies on some assumptions about the internal structures
#      of librmn, and may fail for future libray verisons.
