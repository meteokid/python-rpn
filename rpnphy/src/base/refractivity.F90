...
!-------------------------------------- LICENCE END --------------------------------------
!** s/p refractivity
      subroutine refractivity (d,f,v,dsiz,fsiz,vsiz,ni,nk)
      use phy_options
      use phybus
      implicit none
...
!
      ...
      real,target, intent(in)    :: d(dsiz),f(fsiz)
      real,target, intent(inout) :: v(vsiz)
!
!
!Author
!          Anna Glazer   (Nov 2008)
!          S. Gaudreault (Mar 2012)
!
!Revisions
!
!Object
...
include "thermoconsts.inc"
      ...
      real, pointer, dimension(:)   :: zdct_bh, zdct_count, zdct_lvl, zdct_lvlmax, zdct_lvlmin, zdct_sndmax, zdct_sndmin, zdct_str, zdct_thick, zdct_trdmax, zdct_trdmin, zpmoins
      real, pointer, dimension(:,:) :: zgz_plus, zdct_moref, zsigm, ztplus
!
      zpmoins     (1:ni)     => f( pmoins:)
      zdct_bh    (1:ni)      => v( dct_bh:)
      zdct_count (1:ni)      => v( dct_count:)
      zdct_lvl   (1:ni)      => v( dct_lvl:)
      zdct_lvlmax(1:ni)      => v( dct_lvlmax:)
      zdct_lvlmin(1:ni)      => v( dct_lvlmin:)
      zdct_sndmax(1:ni)      => v( dct_sndmax:)
      zdct_sndmin(1:ni)      => v( dct_sndmin:)
      zdct_str   (1:ni)      => v( dct_str:)
      zdct_thick (1:ni)      => v( dct_thick:)
      zdct_trdmax(1:ni)      => v( dct_trdmax:)
      zdct_trdmin(1:ni)      => v( dct_trdmin:)
      zdct_moref (1:ni,1:nk) => v( dct_moref:)
      zgz_plus   (1:ni,1:nk) => v( gztherm:)
...
!
      ...
      call mfoefq3(ee,d(huplus),ppp,ni,nk,ni)
!
