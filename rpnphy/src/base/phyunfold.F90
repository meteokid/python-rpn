...
contains
   ...
   !/@*
   function phyunfoldmeta(F_dest, F_ijk0, F_ijk1, F_metaplus) result(F_istat)
      implicit none
      !@object Transfer data from p_runlgt space
      !@params
      integer,intent(in) :: F_ijk0(3), F_ijk1(3)
      real,intent(out) :: F_dest(F_ijk0(1):F_ijk1(1),F_ijk0(2):F_ijk1(2),F_ijk0(3):F_ijk1(3))
...
!$omp do
      do j = 1, phydim_nj
         do k = F_ijk0(3), F_ijk1(3)
            ...
            do i = 1, phydim_ni
               F_dest(ijdrv_phy(1,i,j),ijdrv_phy(2,i,j),k) = F_metaplus%ptr(indx0+i,j)
            end do
         end do
      end do
!$omp end do
!$omp end parallel
      ...
      !---------------------------------------------------------------
      return
   end function phyunfoldmeta

end module phyunfoldmeta_mod


!/@*
function phyunfold2(F_dest, F_nomvar_S, F_bus_S, F_ijk0, F_ijk1) result(F_istat)
...
#include <msg.h>
   ...
   F_istat = phyunfoldmeta(F_dest, F_ijk0, F_ijk1, mymetaplus)
   !---------------------------------------------------------------
   return
end function phyunfold2
