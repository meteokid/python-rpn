...
subroutine kfcp7(ix,kx,flagconv,kkfc,psb,tp1,qp1, &
     ub,vb,scr3, &
     ...
     rliq_int,rice_int, &
     rnflx,snoflx, &
     kount,xlat,mg,mlac,critmask,delt)
   use cnv_options
   implicit none
...
!                                thermodynamic functions
      ...
      integer, parameter :: ikfcpcp=1
      real :: rad, cdepth, detreg, dettot, timec, timea
      rad = kfcrad ; cdepth = kfcdepth; detreg = kfcdlev
      dettot = kfcdet ; timec = kfctimec ; timea = kfctimea
...
!PV to refresh every timestep use following line instead of previous
!        IF ( .NOT. TRIGG2 (I)  ) GO TO 325
...
!AZ-PV Cape dependent timec; decrease timec for high cape values
         if(kfctaucape(1) > 0) then
          ...
          NIC   = max(NIC,1)
          TIMEC = FLOAT(NIC) * DELT
          TIMEA = min(TIMEA,TIMEC)   ! time interval for refresh of tendencies
          NIC   = nint(TIMEA/DELT)   ! timestep counter for refresh of tendencies
        endif
...
!141     continue ! PV:use to turn downdraft off
...
117           QD(NDK)=0.
            ...
            DMFMIN=0.
            goto 165
         endif !PV commenter pour enlever le downdraft
...
!PV output liquid and solid rather than sum
...
!PV don't forget dqidt in vertical integral
